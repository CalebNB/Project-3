...
for epoch in range(25000):
    inputs = input_set
    XW = np.dot(inputs, weights)+ bias       #####This feed the data through one neuron 25000 times
    z = sigmoid(XW)                         ######This is the smoothing fuction so the output can be
    error = z - labels                      ######A number between 1 and 0 not just 1 and zero which helps with output swings
...
single_pt = np.array([1,1,0])  #This array represents the persons individual answers to questions. 
result = sigmoid(np.dot(single_pt, weights) + bias) # This person or parents have over a certain value in assets
print(result)                                       # and scored well on the SAT
...
single_pt = np.array([0,1,1])  #This has the required assets and had a high gpa in high school.
result = sigmoid(np.dot(single_pt, weights) + bias)
print(result)
...
single_pt = np.array([0,0,0])  #This array represents the persons individuall answers to questions.
result = sigmoid(np.dot(single_pt, weights) + bias)
print(result)

print ("This is the chance the person E who has money will get accepted to this university")
single_pt = np.array([0,1,0])  #This array represents the persons individuall answers to questions.
result = sigmoid(np.dot(single_pt, weights) + bias)
print(result)
